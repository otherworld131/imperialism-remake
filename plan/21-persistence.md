...
- [x] Snapshot includes: map, all nations, all units, all buildings, all diplomatic relations, tech state, turn number, treasury, warehouse, transport orders, pending orders
...
- [x] Unit tests: save slot management

### Save File Contents
- [x] Game metadata: version, timestamp, player name, nation, difficulty, turn number
- [x] Map state: all tile data (terrain, resources, improvements, infrastructure)
- [x] Nation state: treasury, diplomatic standing, tech research status, building levels
- [x] Unit state: all civilian, army, and naval units with positions, health, medals
- [x] Diplomatic state: all relations, treaties, consulates, embassies
- [x] Economic state: warehouse contents, production assignments, transport allocations
- [x] Trade state: active subsidies, trade history
- [x] AI state: persistent AI memory/goals per nation
- [x] Pending orders: orders submitted for current turn but not yet resolved
- [ ] RNG state: random number generator state for deterministic replay
- [x] Unit tests: every state component serializes and deserializes correctly
...
- [x] Unit tests: load validation catches corrupted data
...
- [x] Export/import saves for sharing
...
- [x] **Autosave test**: Play 5 turns → verify 5 autosave files created
