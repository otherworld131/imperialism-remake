...
use application::queries::{
    FrontendQuery, get_diplomacy_screen, get_industry_screen, get_map_screen, get_trade_screen,
    get_transport_screen,
};
    ...
    match cmd {
        ...
        RecruitArmyUnit {
            ...
            let cap = game
                ...
                .unwrap_or(domain::types::ProvinceId(0));
            ...
            nation
                .military
                .army
                .push(domain::military::units::ArmyUnit::new(
                    id, unit_type, nation_id, cap,
                ));
            CommandResult::success()
        }
        ...
        AssignBeachhead {
            ...
            game.transient.pending_landings.push((
                NationId(nation_id),
        ...
        ExpandBuilding {
            ...
            *nation
                ...
                .or_insert(0) -= lumber_needed;
            *nation
                .economy
                .materials
                .entry(MaterialType::Steel)
                .or_insert(0) -= steel_needed;
            let building = nation
                .economy
                .buildings
                .iter_mut()
        ...
        HireCivilian {
            ...
            if !ct.is_unlocked(
                ...
                return CommandResult::error(
                    "civilian type locked: required technology not researched",
                );
            }
            ...
            nation
                .military
                .civilians
                .push(domain::economy::civilians::Civilian::new(id, ct, nation_id));
            CommandResult::success()
        }
        ...
        SetShipOperation {
            ...
            let _ = game
                ...
                .nations
                .iter()
                .any(|n| n.military.warships.iter().any(|s| s.id == sid));
...
fn respond_to_query(
    ...
    match query {
        MapScreen => {
            ...
            .map_err(|e| {
                application::ApplicationError::invalid(format!("query serialization failed: {e}"))
            })
        }
        TransportScreen => {
...
mod tests {
    ...
    fn queue_unit_move_to_own_province_succeeds() {
        ...
        let cmd = FrontendCommand::QueueUnitMove {
            unit_id,
            target_province: own_province.0,
        };
    ...
    fn hire_civilian_deducts_treasury() {
        ...
        assert_eq!(
            game.get_nation(nid).unwrap().military.civilians.len(),
            before_count + 1
        );
    }
    ...
    fn build_freight_car_deducts_treasury() {
        ...
        let result = apply_command(
            &mut game,
            FrontendCommand::BuildFreightCar { nation_id: nid.0 },
        );
        assert!(result.ok, "{:?}", result.message);
    ...
    fn give_frigate_materials(
        game: &mut domain::game_state::GameState,
        nid: domain::types::NationId,
    ) {
        use domain::types::MaterialType;
    ...
    fn build_warship_goes_to_warships() {
        ...
        assert!(result.ok, "{:?}", result.message);
        assert_eq!(
            game.get_nation(nid).unwrap().military.warships.len(),
            before + 1
        );
    }
    ...
    fn build_merchant_ship_goes_to_merchant_fleet() {
        ...
        assert!(result.ok, "{:?}", result.message);
        assert_eq!(
            game.get_nation(nid).unwrap().military.merchant_fleet.len(),
            before + 1
        );
    }
    ...
    fn setup_two_zone_game() -> (
        GameState,
        domain::map::sea_zones::SeaZoneId,
        domain::map::sea_zones::SeaZoneId,
    ) {
    ...
    fn move_fleet_exhausted_fleet_cannot_move_again() {
        ...
        let r1 = apply_command(
            &mut game,
            FrontendCommand::MoveFleet {
                nation_id: nid.0,
        ...
        assert_eq!(
            budget_after,
            Some(0),
            "exhausted fleet budget must be stored as 0, not absent"
        );

        // Move 2: must fail — zero budget
        let r2 = apply_command(
            &mut game,
            FrontendCommand::MoveFleet {
                nation_id: nid.0,
    ...
    fn set_player_sell_order_stores_order() {
        ...
        let result = apply_command(
            &mut game,
            FrontendCommand::SetPlayerSellOrder {
                nation_id: nid.0,
    ...
    fn accept_nap_proposal_applies_treaty() {
        ...
        let result = apply_command(
            &mut game,
            FrontendCommand::AcceptProposal {
                nation_id: nid.0,
                proposal_index: 0,
            },
        );
        assert!(result.ok, "{:?}", result.message);
        assert!(
            game.world.diplomacy.pending_proposals.is_empty(),
            "proposal should be removed"
        );
        let rel = game.world.diplomacy.get_relation(nid, other).unwrap();
        assert!(
            rel.has_treaty(TreatyType::NonAggressionPact),
            "NAP treaty should be applied"
        );
    }
    ...
    fn expand_building_rejects_while_already_expanding() {
        ...
        let result = apply_command(
            &mut game,
            FrontendCommand::ExpandBuilding {
                nation_id: nid.0,
        ...
        assert!(
            !result.ok,
            "re-entry should be rejected when expansion is in progress"
        );
    }
    ...
    fn accept_peace_calls_queue_peace() {
        ...
        assert!(result.ok, "{:?}", result.message);
        assert!(
            !game.world.diplomacy.is_at_war(nid, other),
            "war should end after peace acceptance"
        );
    ...
    fn accept_proposal_failure_keeps_proposal_in_pending() {
        ...
        assert_eq!(
            game.world.diplomacy.pending_proposals.len(),
            1,
            "proposal should remain on failure"
        );
    }
