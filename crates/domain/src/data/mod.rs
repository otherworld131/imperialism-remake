...
pub struct GameConfig {
    ...
    pub armory_steel_per_arm: u32,
    pub armory_labor_per_arm: u32,
    // Production ratios
    ...
    /// Flat raw FP per entrenched garrison defender — see `game.lua`.
    pub garrison_entrenchment_fp: f64,
    pub battle_attacker_fp_loss_ratio: f64,
    pub battle_defender_fp_loss_ratio: f64,
    /// Role-aware combat (card #478). Range-first-strike: the longer-ranged
    /// side fires one free volley before round 1 from only its over-range
    /// units. See `scripts/config/game.lua` for the alternatives considered.
    pub combat_first_strike_enabled: bool,
    pub combat_first_strike_damage_multiplier: f64,
    /// Round-1-only firepower bonus for attacking cavalry units.
    pub combat_cavalry_charge_bonus: f64,
    /// AI strength estimator (never consumed by the resolver).
    pub combat_ai_strength_lanchester: bool,
    pub combat_ai_strength_range_advantage_coeff: f64,
    pub combat_ai_strength_range_advantage_cap: f64,
...
impl Default for GameConfig {
    fn default() -> Self {
        Self {
            ...
            armory_steel_per_arm: 1,
            armory_labor_per_arm: 2,
            ...
            chain_priority_weights: [0.8, 1.0, 0.8, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5],
            gold_value: 500,
            gems_value: 1000,
...
pub fn default_ship_stats() -> HashMap<ShipType, ShipStats> {
    ...
    [
        (
            Trader,
            s(0, 0, 0, 25, 0, 2, merchant, 2, 4, 0, 0, 0, 1, None),
        ),
        (
            Indiaman,
            s(0, 0, 5, 40, 0, 4, merchant, 3, 7, 0, 0, 0, 1, None),
        ),
        (
            Clipper,
            s(
        ...
        (
            Frigate,
            s(3, 5, 10, 35, 2, 0, warship, 2, 5, 2, 0, 0, 1, None),
        ),
        (
            ShipOfTheLine,
            s(6, 6, 20, 65, 2, 0, warship, 3, 8, 5, 0, 0, 1, None),
        ),
        (
            Raider,
            s(
